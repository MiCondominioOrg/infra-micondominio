...
## @params: [JOB_NAME]
args = getResolvedOptions(sys.argv, ['JOB_NAME']) #Error Category: INVALID_ARGUMENT_ERROR; Failed Line Number: 6; NameError: name 'getResolvedOptions' is not defined
...
# Diccionario que mapea nombre de archivo a su primary key
primary_keys = {
    "cuotas": "id_cuota",
...
for obj in bucket.objects.filter(Prefix=route):
    key = obj.key
    if key.endswith('.csv'):
        # Extraer solo el nombre del archivo sin el .csv
        table = key.split('/')[-1].replace('.csv', '')
        nombres_archivos.append(table)
        print("Fichero: ", table)
